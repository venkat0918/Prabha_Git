...
LoanId_List=[
'fd595828-dcc9-491c-a807-f3caad7abb37',
...
for id in LoanId_List:
    url = "https://na1.ai.dm-us.informaticacloud.com/active-bpel/rt/p_API_ssvr_Loan_Stipdetails"
    payload=('{"LoanGuid":"'+id+'"}')
    headers = {
    'Authorization': 'Basic aW5mb3JtYXRpY2EtcHJkLmR3QHBubWFjLmNvbTpsQ01XdE9rSWRIczdmejV5',
    'Content-Type': 'application/json'
    }
    response = requests.request("POST", url, headers=headers, data = payload)
    if 'False' in str(response.text.encode('utf8')):
        unprocessedList.append(id)
    else:
        print(str(response.text.encode('utf8'))+'   '+id)
for u_id in unprocessedList:
    with open ('C:\\Users\\dell\\Desktop\\Python\\Code_Prctise\\unprocessed_stip.txt','a') as a:
        a.write(u_id+'\n')
